...
describe('transaction model tests', () => {
    ...
    test('to get transaction function test', async () => {
        const senderAddress = 'aura1w0ten4x8faffehzsjuae3v9jj5xt45qutwk0te'
        const receiverAddress = 'aura1xc67705clhg7ftfa0khmzq7z7kx87x7mrzlvuw'
        // get transaction with right tx hash
        expect((await getTransaction(senderAddress, 'deposit')).length).toBeGreaterThan(0)
        expect((await getTransaction(receiverAddress, 'withdraw')).length).toBeGreaterThan(0)
